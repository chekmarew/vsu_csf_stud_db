...
def api_schedule_exams():
    ...
    if "archive_year" in request.args and "archive_semester" in request.args:
        try:
            ...
            return jsonify({"ok": False, "error": "Неверное значение GET-параметра 'archive_year' или 'archive_semester'"}), 400
    ...
    result = {
        ...
        "classroms": [r.classroom for r in db.session.query(Classroom.classroom).order_by(Classroom.classroom)]
    }
    min_session_start_date, max_session_end_date = None, None
    for g in groups:
        g: StudGroup = g
        if g.session_start_date is None or g.session_end_date is None:
            continue
        ...
        g_j = {
            ...
            "session_start_date": g.session_start_date.isoformat(),
            "session_end_date": g.session_end_date.isoformat(),
            "curriculum_units": [],
        ...
        for cu in g.curriculum_units:
            ...
            if cu.mark_type != 'exam' and not cu.has_simple_mark_exam:
                continue

            if cu.subject_id not in subject_ids:
                subject_ids.add(cu.subject_id)
                s: Subject = cu.subject
            ...
            cu_j = {
                ...
                "exams": [exam_to_json(exam) for exam in cu.exams]
            }
        ...
        if min_session_start_date is None or g.session_start_date < min_session_start_date:
            min_session_start_date = g.session_start_date

        if max_session_end_date is None or g.session_end_date > max_session_end_date:
            max_session_end_date = g.session_end_date

        result["stud_groups"].append(g_j)

    # Sorts
    result["subjects"].sort(key=lambda s: s["name"])
    result["teachers"].sort(key=lambda t: (t["surname"], t["firstname"], t["middlename"]))

    if min_session_start_date is not None and max_session_end_date is not None:
        result["min_session_start_date"] = min_session_start_date.isoformat()
        result["max_session_end_date"] = max_session_end_date.isoformat()
        holidays = [h.date for h in db.session.query(Holiday).filter(Holiday.date >= min_session_start_date).filter(Holiday.date <= max_session_end_date).all()]
        result["holidays"] = []
        d = min_session_start_date
        while d <= max_session_end_date:
            if d.isoweekday() == 7 or d in holidays:
                result["holidays"].append(d.isoformat())
