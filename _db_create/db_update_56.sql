...
ADD COLUMN `kind_of_study_activity` ENUM('student', 'student_foreign') NOT NULL DEFAULT 'student' AFTER `financing`;
ALTER TABLE `student`
ADD COLUMN `kind_of_study_activity` ENUM('student', 'student_foreign') NOT NULL DEFAULT 'student' AFTER `financing`;
...
VIEW `student_vw` AS
    SELECT
        ...
        `s`.`kind_of_study_activity` as `kind_of_study_activity`
    FROM
        (((`student` `s`
