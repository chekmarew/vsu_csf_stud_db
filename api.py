...
from model import CurriculumUnit, Teacher, Student, Person, AttMark, Department, StudGroup, Specialty, LessonStudent, Lesson
...
def _att_marks_email_alerts(cu: CurriculumUnit):
    if mail is not None and cu.status == 'exam' and cu.result_failed is not None and cu.result_failed > app.config['RESULT_FAIL_WARNING']:

        teachers = db.session.query(Teacher).join(Person, Teacher.person_id == Person.id).filter(Teacher.active).filter(Person.email.isnot(None)).filter(or_(Teacher.id == cu.teacher_id, Teacher.dean_staff, and_(Teacher.department_leader, Teacher.department_id == cu.teacher.department_id))).all()

        msg = Message(subject=app.config['MAIL_SUBJECT'],
                      recipients=[t.person.email for t in teachers])
        msg.html = render_template('email_alerts/att_marks.html', curriculum_unit=cu)
        try:
            mail.send(msg)
            return True
        except socket.error:
