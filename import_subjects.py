...
with app.app_context():
    ...
    for row in f:
        # Б1.О.02;История России;2;09.03.02;Информационные системы и технологии;;Дифференцированный зачет;144;Лавлинский;Сергей;Александрович;доцент (к.н., доцент);406
        subject_code, subject_name, semester, s_code, s_name, specialization, mark_type, hours, t_surname, t_firstname, t_middlename, t_rank, department_id = tuple(x.strip() for x in row.strip().split(";"))
        hours = int(hours)
        ...
        stud_groups_q = db.session.query(StudGroup).join(Specialty, StudGroup.specialty_id == Specialty.id).filter(StudGroup.active).filter(StudGroup.semester == semester).filter(Specialty.code == s_code)
        if specialization:
